...
class StreamCountOnline(host:String, port:Int) extends Receiver[String](StorageLevel.DISK_ONLY){
  //启动的时候调用
  override def onStart(): Unit = {
    println("启动了")
    var esData = esSearch("now-2h")
  ...
  def esSearch(gte : String): Iterator[String] = {
    ...
    val strings: Iterator[String] = Iterator("Baidu", "Google", "Runoob", "Taobao")
...
object StreamCountOnline {
  def main(args: Array[String]): Unit = {
    ...
    //从socket接收数据
    val lineDStream = ssc.receiverStream(new StreamCountOnline("192.168.19.123",6789))
    //逻辑处理
